...
def write_model_info(args, log_dir):
    ...
    lines.append('-' * 68)
    lines.append('  COORDINATE SPACE & REWARD')
    lines.append('-' * 68)
    lines.append('  x and y values MUST be between 0.0 and 1.0')
    lines.append('  These represent normalized GPS coordinates.')
    lines.append('  Training: random [0,1] coords with on-the-fly time matrices.')
    lines.append('  Testing:  pre-generated OSRM data (real driving times, seconds).')
    lines.append('')
    lines.append('  REWARD = total travel time in SECONDS (lower = better)')
    lines.append('  The model minimizes total driving time, NOT Euclidean distance.')
    lines.append('')
    lines.append('  To normalize real GPS coords for testing:')
    lines.append('    x = (lon - lon_min) / (lon_max - lon_min)')
    lines.append('    y = (lat - lat_min) / (lat_max - lat_min)')
    lines.append('')
